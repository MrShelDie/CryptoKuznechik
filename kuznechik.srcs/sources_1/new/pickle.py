keys = [0x95d517afb355e5897dc0862e516a8f40,
        0x2d79bd4575812772b2c7d0ddd5945a6e,
        0xa3c84d42a8827dc0e3171d589d042bf6,
        0x34299adaae4ae0311a909ca03b94c7f0,
        0x78176a5a752b812758e9a019a940e95c,
        0x1a47cd2ae5457fea9ba7825b7d847dd3,
        0x2eecdc76d4bcb16ab9a27f5b8cbfd36a,
        0x0a84fe291be3fe436336c6a02d83b9a2,
        0x0d22eea721cf2b19eecb8d1d46fa6c93,
        0xc8efc87b8304dc0585d243063fbcc236]
...
inv_S = [0xA5, 0x2D, 0x32, 0x8F, 0x0E, 0x30, 0x38, 0xC0,
...
def k_round(value: int, key: int, iteration: int) -> int:
    ...
    #print(xor_bytes_list)

    for stage in range(16):
        last_byte = 0
        for i, b in enumerate(xor_bytes_list):
            if L[i] == 1:
                last_byte = last_byte ^ b
            else:
...
def k_chipher(msg: int) -> int:
    for i in range (10):
        msg = k_round(msg, keys[i], i)
        print("{:x}".format(msg))
    return msg


def inv_k_round(value: int, key: int, iteration: int) -> int:
    if iteration == 9:
        return value ^ key

    bytes_list = list(value.to_bytes(16, "little"))
    for stage in range(16):
        first_byte = 0
        for i, b in enumerate(bytes_list):
            if L[i] == 1:
                first_byte = first_byte ^ b
            else:
                first_byte = first_byte ^ mul_tables[sorted_unique_L.index(
                    L[i])][b]
        del bytes_list[-1]
        bytes_list.insert(0, first_byte)

    for i, b in enumerate(bytes_list):
        bytes_list[i] = inv_S[b]

    value = int.from_bytes(bytes_list, "little")
    return value ^ key


def k_dechipher(msg: int) -> int:
    for i in reversed(range(10)):
        msg = inv_k_round(msg, keys[i], i)
    return msg
...
def mul(a: int, b: int) -> int:
    c = 0
    for i in range(8):
        if b % 2:                   # если младший бит равен единице
            c = c ^ a
        shifted_a = (a * 2) % 256   # а, сдвинутая на 1 влево
        if a >= 128:                # если старший бит равен единице
            a = shifted_a ^ 0xc3    # порождающий полином x^8 + x^7 + x^6 + x + 1
        else:
            a = shifted_a
        b = b // 2                  # b сдвинутая на 1 вправо
    return c

def L_stage(val):
    xor_bytes_list = list(val.to_bytes(16, "big"))
    last_byte = 0
    for i, b in enumerate(xor_bytes_list):
        last_byte = last_byte ^ mul(b, L[i])
    del xor_bytes_list[-1]
    xor_bytes_list.insert(0, last_byte)
    return int.from_bytes(xor_bytes_list, "big")
...
if __name__ == '__main__':
    msg = 0x3ee5c99f9a41c389ac17b4fe99c72ae4

    ciphered_msg = k_chipher(msg)

    # ciphered_msg = L_stage(0x820585051c93effc1bfe02e1309f00)

    # msg = 0x64a59400000000000000000000000000
    # for i in range(16):
    #     msg = L_stage(msg)
    # print("{:x}".format(msg))
    print("{:x}".format(ciphered_msg))
